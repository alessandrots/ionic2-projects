...
app.get('/rest/friends', function (req, res, next) {
    responder(res, friend_1.getFriends(req.params));
});
...
var httpServer = app.listen(8000, ip, function () {
...
function responder(res, dados) {
    ...
    if (random > 7) {
        setTimeout(function () { return res.status(404).send('Not found'); }, 1000 + (100 * (Math.floor(Math.random() * 10) + 1)));
    }
    else {
        setTimeout(function () { return res.json({ dataAtualizacao: new Date(), data: dados }); }, 1000 + (100 * (Math.floor(Math.random() * 10) + 1)));
    }
}
//# sourceMappingURL=coordenador.js.map